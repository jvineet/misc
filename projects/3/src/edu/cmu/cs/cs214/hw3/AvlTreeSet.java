...
public class AvlTreeSet {
	 ...
	 * integer. This method will return false if the integer is not in the tree
	...
	public int getMax() {
		if (isEmpty()) {
			throw new IllegalStateException(
					"Tried to get maximum item of an empty AvlTreeSet");
		}
		return mRoot.getMax();
	}
	...
	public int getMin() {
		if (isEmpty()) {
			throw new IllegalStateException(
					"Tried to get minimum item of an empty AvlTreeSet");
		}
		return mRoot.getMin();
	}
	...
	private static class Node {
		...
		public Node insert(int value) {
			if (!isEmpty()) {
				...
				if (value <= mValue) {
					return new Node(mValue, mLeft.insert(value), mRight)
							.balance();
				}

				if (value > mValue) {
					return new Node(mValue, mLeft, mRight.insert(value))
							.balance();
				}
			}
		...
		public Node remove(int value) {
			if (isEmpty()) {
				throw new IllegalStateException(
						"Tried to remove an item not in the AvlTreeSet");
			}
			...
			if (value == mValue) {
				...
				return new Node(leftMax, mLeft.remove(leftMax), mRight)
						.balance();
			}
		...
		private Node balance() {
			...
			if (rightHeight > leftHeight + 1) {
				...
				if (mRight.getRight().getHeight() > mRight.getLeft()
						.getHeight()) {
		...
		private Node doubleRotateRight() {
			...
			Node left = new Node(mLeft.getValue(), mLeft.getLeft(), mLeft
					.getRight().getLeft());
		...
		private Node doubleRotateLeft() {
			...
			Node right = new Node(mRight.getValue(), mRight.getLeft()
					.getRight(), mRight.getRight());
			return new Node(value, left, right);
		}
